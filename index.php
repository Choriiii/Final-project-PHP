...
function editProduct(int $productID, string $name, string $description, float $price, array $image)
{
    $timestamp = date("Y-m-d H:i:s");
    $ip = $_SERVER['REMOTE_ADDR'];
    $userAgent = $_SERVER['HTTP_USER_AGENT'];
    ...
    if ($imagePath) {
        ...
        $updateStmt->bind_param("ssdsi", $name, $description, $price, $imagePath, $productID);
    } else {
